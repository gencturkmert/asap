...
def get_model(normalization_layer=''):
    ...
    model = tf.keras.Sequential(
        [
            ...
            tf.keras.layers.GRU(256,return_sequences=True, stateful = True,activation = 'tanh'),
            tf.keras.layers.Dropout(0.5),
            tf.keras.layers.GRU(128,return_sequences=True, stateful = True,activation = 'tanh'),
            tf.keras.layers.Dropout(0.5),
            tf.keras.layers.GRU(64,return_sequences=True, stateful = True,activation = 'tanh'),
            tf.keras.layers.Dropout(0.5),
            tf.keras.layers.TimeDistributed(Dense(1,activation = "sigmoid"))
        ]
    )
...
sweep_id = wandb.sweep(sweep_config, project="EEG_v5")
#sweep_id = "vazy0ogh" # to continue a sweep
import time
...
wandb.agent(sweep_id, project="EEG_v5", function=train)